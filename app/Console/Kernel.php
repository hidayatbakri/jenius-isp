...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule): void
    {
        ...
        $schedule->call(function () {
            ...
            $customers = Transaction::with('customer')->where('olt_id', $getactiveolt->id ?? 0)->groupBy('customer_id')->get();

            $now = Carbon::today()->format("Y-m-d");
            foreach ($customers as $row) {
                if ($row->customer->expire <= $now && $row->customer->expire != null) {
            ...
            if ($getactiveolt != null) {
                ...
                if ($result == 0) {
                    $dataOlt = $telnet->getPowerFromArrayOfData($customers);
                    $telnet->Disconnect();
                    $data =  $dataOlt;
                }
            }
